...
import {
  ...
  setBossActivation,
  plate2Location,
  detectAndRegisterBossTrigger,
} from "../interactions/boss";
...
import { detectAndTriggerRPGLikeTalkingPerPhrase } from "../interactions/rpgLikeTalkingPerPhrase";
import { detectAndTeleportUnauthorizedUser } from "../interactions/teleportUnauthorizedUser";
import { blockAccessToUnauthorizedUser } from "../interactions/blockAccessToUnauthorizedUser";
import { Position } from "../types";

const playerPosition: Record<string, Position> = {};

export function onPlayerMoves(
  data: ServerClientEventByCase<"playerMoves">,
  ...
  detectAndTriggerRPGLikeTalkingPerPhrase(playerNewPosition, mapId);
  detectAndTeleportUnauthorizedUser(playerNewPosition, mapId, playerId);
  blockAccessToUnauthorizedUser(playerNewPosition, mapId, playerId);
  detectAndRegisterMovementRanking(
    playerNewPosition,
    playerPosition,
    mapId,
    playerId
